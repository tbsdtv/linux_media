...
//  SIMD_ALIGNED(uint8_t row[MAXTWIDTH * 4]);
 ...
 SIMD_ALIGNED(uint16_t row00[MAXTWIDTH]);
...
void InterpolateRow_16To8_AVX2(uint8_t* dst_ptr,
 ...
 // SIMD_ALIGNED(uint16_t row[MAXTWIDTH]);
  while (width > 0) {
    int twidth = width > MAXTWIDTH ? MAXTWIDTH : width;
    InterpolateRow_16_C(row00, src_ptr, src_stride, twidth, source_y_fraction);
    Convert16To8Row_AVX2(row00, dst_ptr, scale, twidth);
    src_ptr += twidth;
    dst_ptr += twidth;
    width -= twidth;
  }
